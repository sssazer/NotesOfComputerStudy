...
- 悲观锁：认为共享数据每次被访问都会出现问题，所以每次获取资源都会上锁

  Java的ReentraintLock和Synchronized等独占锁就是悲观锁

- 乐观锁：乐观锁认为共享数据被访问不会出现问题，所以不会实际加锁。只是在提交修改时验证对应的资源是否被其他线程修改了，如果被修改就驳回此次提交修改。
