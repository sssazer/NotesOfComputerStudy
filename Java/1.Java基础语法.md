...
项目部署到服务器时，需要将程序打成jar（java archive file）包再上传到服务器，这样服务器上有jre环境之后就可以直接运行。

如果想将写好的程序发给别人使用，需要进一步将jar包打包为exe文件

**idea打包**

打jar包

1. idea工具栏 - file - project structure - artifacts - add(点击加号) - JAR - from modules with dependencies

2. 选择程序入口main class，在下面的JAR files from libraries中选择第二项，之后点击确认。

   第一项会打成一个jar包，但是会有依赖问题；第二项会生成一个文件夹，所有依赖jar包都会放在这个文件夹内，此时jar包可以正常执行。

3. idea工具栏 - build - build artifacts - 会弹出所有的jar file - 选择 - build

打exe文件

需要借助exe4j，将idea中打出的多个jar包打入一个exe中

1. 启动exe4j，**Choose project type**页面选择Regular mode（为了导入多个jar包）

2. **Configure application**页面中：指定项目名称和Distribution source directory

   distribution source direction指的是要打包的所有文件和资源（比如jar包）所在的文件夹

3. **Configure executable**页面中：选择项目类型（GUI application），Executable name输入生成的exe文件的名称，Icon File可以指定exe文件的图标

   高级选项中选择Archtecture（32/64/ARM），之后下一步，选择64-bit

4. **Configure Java invocation**页面：VM Parameters中输入`-Dfile.encoding=utf-8`

   class path右侧点击加号，Entry Type选择Scan directory，下面Directory中选择idea打出的所有jar包的文件夹。Main class中选择启动项

5. **Configure JRE**页面：输入最小jdk版本

之后无脑下一步即可完成

**maven打包**

maven打jar包可以打成一个包含所有依赖的jar包
...
比较重要的部分是maven-assembly-plugin的依赖，启动类全路径名，和jdk版本

之后在console控制台中进入到项目路径下，执行`mvn clean package assembly:single`命令。执行完成后即可在target目录下看到两个jar包，其中后缀为jar-with-dependencies的是带依赖的jar包。

利用exe4j将单个jar包打成exe和上述步骤相同，不过是在Choose project type页面选择JAR in EXE mode
