...
字符串是Reference Type，但是并没有保存在堆区，而是保存在方法区的常量池中。在常量池中存放的是char数组

这也说明字符串是常量，无法直接修改值。当修改字符串时实际上是在常量池中创建了一个新的字符串常量并将新的地址返回给栈上的变量。
...
当创建字符串变量时，会在常量池中检查是否有相同内容的char数组，如果有就直接将数组的地址放入栈区；否则就在常量池中创建该内容的char数组，再将数组地址存入栈区变量中。
...
当使用new创建字符串时，会先在堆区创建一个String对象，栈中的变量保存这个堆区对象的地址。堆区的String对象中不会直接保存String字符串内容，字符串的内容依然是保存在方法区常量池的char数组中，堆区String对象保存的是char数组地址。
