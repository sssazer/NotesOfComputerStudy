...
先在配置中配置好sync所需的队列、交换机、以及将二者绑定，配置jackson2jsonmessageconverter，将消息队列传来的json数据转为DTO对象

监听消息，获取到消息中DTO对象

根据远程仓库地址将代码pull到本地

根据受影响文件信息创建对应的BlogVO对象

根据不同修改类型新增博客或者修改博客 （可以用文件夹名当标签）

- 修改博客

  使用AdminFeignClient，调用mogu_admin的BlogRestApi中的editBlog接口

- 新增博客

  使用AdminFeignClient，调用mogu_admin的uploadLocalBlog接口
