...
List相当于引用类型数据，当直接用例如`listb = lista`这样的赋值语句赋值时，或者将lista作为参数传入函数时，对listb和在函数中的操作都会直接影响lista
...
### 4.1.4 List is Mutable

这一小节列举以下在哪些情况下会创建新的List对象，在哪些情况下会直接使用原有List对象
