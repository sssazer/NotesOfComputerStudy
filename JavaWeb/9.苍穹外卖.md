...
数据传输对象DTO（Data Transfer Object），是面向前端的数据传输对象，比如前端在传回登录、注册数据和后端向前端回传要展示的数据时，可能并不会包括实体类中的所有属性。
...
![JWT流程](D:\CS_Source\myNote-of-ComputerStudy\JavaWeb\9\JWT流程.png)
