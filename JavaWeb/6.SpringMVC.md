...
应用分层是一种软件开发设计思想，它将应用程序分为N个层次，这N个层次分别负责各自的职责，多个层次之间协同提供完整的功能

常见的三层架构、MVC架构等就是应用分层的具体体现

一般来说一个完整Java Web应用程序的结构如下图：

![image](images/6.SpringMVC/3033489-20230206120852685-1154196572.png)

三层架构和MVC结构就是在这些结构的基础上再分层

### 1.1.1 三层架构

三层架构将系统划分为三层：

![image](images/6.SpringMVC/3033489-20230206121104221-1837440785.png)

- 表示层（UI）

  用来实现与用户的交互，接收用户请求，并将请求交给业务逻辑层（BLL）和数据访问层（DAL）进行处理，最后将处理结果返回给用户。

- 业务逻辑层（BLL）

  起到承上启下的作用，接收表示层传递来的请求，并针对业务对数据进行处理，以实现业务目标。

- 数据访问层（DAL）

  用于实现与数据库的交互和访问，例如从数据库中获取数据、保存或修改数据库中的数据等。

### 1.1.2 MVC架构

MVC，全称Model-View-Controller

![image](images/6.SpringMVC/3033489-20230206120906285-1373540775.png)
...
Spring MVC是MVC模式在Spring中的具体实现，并且简化了Java Web应用在MVC模式下的开发，比如：

- 将Controller控制器层进一步分为前端控制器DispatcherServlet和后端控制器Controller，并且完成了对DispatcherServlet的自动实现，使得开发者可以专注于后端控制器Controller的实现
- 将Model拆分成业务层Service和数据访问层DAO
- 在视图层完成了对多种视图的支持，经过简单配置即可使用，比如thymeleaf、JSP、FreeMarker等
- 对Controller层、Service层和DAO层都提供了对应的注解简化开发
...
当浏览器发送的请求等于 @RequestMapping(value = "") 中value属性的值时，映射到对应方法，并调用该方法，将该方法返回值返回给浏览器
