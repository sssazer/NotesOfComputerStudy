...
为什么叫动态代理，实际上就是自动帮你创建了代理对象，不用你自己手动创建代理类再创建代理对象。
...
- 创建JDK动态代理，在add方法执行前后都输出一句话
  ...
  - InvocationHandler，这是一个接口，接口中需要实现invoke方法

    当使用创建的代理对象调用方法时，都会被拦截并调用这里的invoke方法，再invoke方法中调用被代理对象的原始方法，就可以达到AOP的效果
